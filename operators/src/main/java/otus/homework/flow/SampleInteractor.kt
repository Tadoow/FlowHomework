...
class SampleInteractor(
    ...
    fun task2(): Flow<String> {
        return sampleRepository.produceNumbers()
            .flatMapConcat { number ->
                when {
                    number % 15 == 0 -> flowOf("$number", "FizzBuzz")
                    number % 5 == 0 -> flowOf("$number", "Buzz")
                    number % 3 == 0 -> flowOf("$number", "Fizz")
                    else -> flowOf("$number")
                }
            }
    }
    ...
    fun task4(): Flow<Int> {
        return sampleRepository.produceNumbers()
            .catch { exception ->
                if (exception is IllegalArgumentException) emit(-1) else throw exception
            }
